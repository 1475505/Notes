...
1. 在DFS中，如何保证无向树的每个节点只递归访问一次？
   答：通用做法是用一个vis数组标记访问过的点，如果某个点之前访问过，就不再递归访问。但对于树来说，一直向下递归，并不会遇到之前访问过的点，所以不需要vis数组。**无向树，除了根节点以外，其余每个点的邻居都包含其父节点，所以要避免访问父节点。我们可以定义dfs(x,fa)表示递归到节点x且x的父节点为fa。只要x的邻居y!=fa,就可以dfs(y,x)向下递归了。**
